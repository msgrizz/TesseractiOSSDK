Pod::Spec.new do |s|
  s.header_dir		    = 'TesseractOCR'
  s.name                    = 'TesseractiOSSDK'
  s.version                 = '4.0.2'

  s.summary                 = 'Use Tesseract OCR in iOS projects written in either Objective-C or Swift.'

  s.homepage                = 'https://github.com/msgrizz/TesseractiOSSDK'
  s.documentation_url       = 'https://github.com/gali8/Tesseract-OCR-iOS/blob/master/README.md'

  s.license                 = { :type => 'MIT',
                                :file => 'LICENSE.md' }

  s.authors                 = { 'Daniele Galiotto' => 'genius@g8production.com',
                                'Kevin Conley' => 'kcon@stanford.edu'}

  s.source                  = { :git => 'https://github.com/msgrizz/TesseractiOSSDK.git', :tag => s.version.to_s }

  s.platform                = :ios, "8.1"
  s.source_files            = 'TesseractOCR/*.{h,m,mm,hpp}', 'TesseractOCR/include/**/*.h', 'TesseractOCR/include/**/*.hpp'
  s.private_header_files    = 'TesseractOCR/include/**/*.h'
  s.requires_arc            = true
  s.frameworks              = 'UIKit', 'Foundation'

  s.ios.deployment_target   = "7.0"
  s.ios.vendored_library    = 'TesseractOCR/lib/*.a'
  s.xcconfig                = { 'OTHER_LDFLAGS' => '-lstdc++ -lz',
                                'CLANG_CXX_LIBRARY' => 'compiler-default' }

end
